// src/components/AthleteProfile/AboutMe.jsx
import React from 'react';
import { FaUserGraduate, FaStar, FaGraduationCap } from 'react-icons/fa';

const AboutMe = ({ athlete }) => {
    ...
    <section className="container mx-auto px-4 md:px-8 py-8 md:py-12 bg-white rounded-lg shadow-md mt-6">
      ...
      <div className="flex flex-wrap gap-3">
        {athlete.skills.map((skill, index) => (
          <span
            key={index}
            className="bg-gamepulse-blue/10 text-gamepulse-blue px-4 py-2 rounded-full text-sm md:text-base font-semibold flex items-center shadow-sm"
          >
        ...
        <>
          ...
          <div className="space-y-3">
            {athlete.digitalClassroomProgress.map((module, index) => (
              <div
                key={index}
                className={`p-4 rounded-lg border-l-4 shadow-sm flex items-center
                  ${module.completed
