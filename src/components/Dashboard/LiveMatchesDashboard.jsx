        ...
        const interval = setInterval(() => {
            setLiveMatches(prevMatches => prevMatches.map(match => {
                ...
                if (isTodayOrFuture && match.status === 'scheduled') {
                    ...
                    if (newGameTime % 15 === 0 && Math.random() > 0.7) { // Roughly every 15 minutes, 30% chance of score change
                        if (Math.random() > 0.5) { newScore1 += 1; } else { newScore2 += 1; }
                    }

                    // Simulate match finishing after a certain time (e.g., 90 minutes for football)
                    if (newGameTime >= 90) {
...
const LiveMatchesDashboard = () => {
    ...
    // UPDATED: Added 'slug' to playerPerformance data
    const playerPerformance = [
        { id: 'pp1', name: 'Chisom Ugo', team: 'Lagos Dynamos', pos: 'FW', goals: 7, assists: 3, rating: 8.9, slug: 'chisom-ugo' },
        { id: 'pp2', name: 'Ama Adom', team: 'Accra Lions', pos: 'MF', goals: 4, assists: 6, rating: 8.7, slug: 'ama-adom' },
        { id: 'pp3', name: 'Thabo Mbeki', team: 'Cape Town FC', pos: 'DF', goals: 1, tackles: 15, rating: 8.5, slug: 'thabo-mbeki' },
        { id: 'pp4', name: 'Zara Conteh', team: 'Nairobi Stars', pos: 'GK', saves: 20, cleanSheets: 5, rating: 8.8, slug: 'zara-conteh' },
    ];
    ...
    const FeaturedMatch = ({ match }) => {
        ...
        // Ensure that the 'slug' is available for the featuredPlayer to pass to the Link
        ...
        return (
            <div className="bg-neutral-dark-gray rounded-xl p-6 relative overflow-hidden h-[400px] flex flex-col justify-between">
                ...
                <div className="relative z-10 bg-neutral-black/40 backdrop-blur-sm rounded-lg p-4 mt-4 flex items-center justify-between">
                    ...
                    {/* CORRECTED LINK: Use featuredPlayer?.slug here */}
                    <Link to={`/athlete/${featuredPlayer?.slug}`} className="bg-gamepulse-yellow text-gamepulse-dark px-5 py-2 rounded-full font-bold hover:bg-gamepulse-yellow/80 transition-colors">
                        View Profile
                    </Link>
                </div>
            </div>
